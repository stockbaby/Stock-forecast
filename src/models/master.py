...
def _top1_margin_loss(pred, target, config: MasterTrainConfig, torch_module) -> object:
    if pred.numel() < 2:
        return pred.new_tensor(0.0)
...
def _daily_group_top1_margin_loss(pred, target, date_codes, config: MasterTrainConfig, torch_module) -> object:
    losses = []
    for code in torch_module.unique(date_codes):
        mask = date_codes == code
        if int(mask.sum().item()) < 2:
...
def _top_label_weighted_listnet_loss(pred, target, config: MasterTrainConfig, torch_module) -> object:
    if pred.numel() < 2:
        return pred.new_tensor(0.0)
...
def _daily_group_top_label_listnet_loss(pred, target, date_codes, config: MasterTrainConfig, torch_module) -> object:
    losses = []
    for code in torch_module.unique(date_codes):
        mask = date_codes == code
        if int(mask.sum().item()) < 2:
            continue
        losses.append(_top_label_weighted_listnet_loss(pred[mask], target[mask], config, torch_module))
    if not losses:
        return pred.new_tensor(0.0)
    return torch_module.stack(losses).mean()
