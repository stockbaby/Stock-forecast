...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a TimeXer-style exogenous patch backbone.")
    parser.add_argument("--config", default="configs/timexer_alpha_fast.yaml")
    ...
    if recent_train_days:
        ...
        if len(train_dates) > int(recent_train_days):
            train_df = train_df[train_df["date"] >= train_dates[-int(recent_train_days)]].copy()
    ...
    train_cfg = TimeXerTrainConfig(
        ...
        early_stopping_patience=cfg["deep"].get("early_stopping_patience", 2),
        lr_decay_factor=cfg["deep"].get("lr_decay_factor", 0.5),
        min_lr=cfg["deep"].get("min_lr", 1e-5),
        regression_weight=cfg["deep"].get("regression_weight", 0.65),
        rank_weight=cfg["deep"].get("rank_weight", 0.2),
        corr_weight=cfg["deep"].get("corr_weight", 0.15),
    ...
    Path(cfg["output"]["metrics_path"]).write_text(json.dumps(metrics, ensure_ascii=False, indent=2), encoding="utf-8")
